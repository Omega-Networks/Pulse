...
enum WebAudit {
    ...
    enum Event: Sendable, Equatable {
        ...
        /// An operator removed a block (unblocked a host) from the trust
        /// settings; the next connection is a fresh first sight.
        case unblocked(host: String, port: Int)
    ...
    static func emit(_ event: Event) {
        ...
        case let .unblocked(host, port):
            trustLogger.warning("web.trust.unblocked host=\(host, privacy: .public) port=\(port, privacy: .public)")
...
extension WebAudit {
    ...
    static func unblocked(host: String, port: Int) { emit(.unblocked(host: host, port: port)) }
