...
- **The window and view mirror the SSH terminal.** `DeviceWebScene` (shipped at W2b as `WindowGroup("Device Web", id: "device-web", for: Device.ID.self)`) mirrors `SSHTerminalScene`, registered in `PulseApp.swift` after it, with the same string-`id:` routing discipline. The routing was later hardened so that, per ADR 0001 §9, all three scenes (SSH Terminal, Site View, Device Web) key on nominal target structs (`DeviceWindowTarget`, `SiteWindowTarget`) rather than the raw `Int64` id shown above; see the Slice 8b amendments summary for the nominal-struct hardening. `DeviceWebView` mirrors `SSHTerminalView`: a `@Query` device-by-id, a `WebView(page)` driven by a `@State WebPage` built in `.task`, a toolbar bound to the page's `@Observable` `title` / `estimatedProgress` / `isLoading` / `backForwardList`, and a `WebLoadStatus` pure helper for the status label.
