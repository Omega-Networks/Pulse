...
- **§4 symmetry hazard at `SSHCredentialsSettings.swift:399-404`.** The credential-delete cleanup nils only `defaultCredentialID`; `defaultUsername` is left set. Documented in-line at both the cleanup site and the `clearDeviceDefaults` helper's doc-comment so the asymmetry is visible to the next maintainer rather than implicit. A device with `defaultUsername = "admin"` and `defaultCredentialID = nil` falls back to the form on next open (the auto-fire gate requires both fields), so the half-cleared state is recoverable; the form's "Clear saved defaults" button is the operator's escape hatch for the leftover username. A follow-up slice will nil both fields at the cleanup site for symmetry with the helper.
