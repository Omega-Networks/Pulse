...
- **`.explicitlyDistrusted` is reserved, not dead.** The evaluator honours it (a hard reject), but no web UI writes a web distrust yet, and a mismatch Reject cancels the challenge without persisting anything. The one operator-visible consequence is deliberate: rejecting a mismatched certificate is not remembered, so the prompt recurs on the next visit. A future Settings surface (per §5) would let an operator record a standing distrust.
